...
for k in range(0,10):
    ...
    star = str(j)
    M_star = binary_df["Companion_mass"][j] # in M_sun
    R_star = binary_df["Companion_radius"][j] * 1/const.c.value # in R_sun / c
    M_bh = binary_df["BlackHole_mass"][j] # in M_sun
    print(M_star, R_star)

    gridsize = 100

    m_pbh_values = np.logspace(np.log10(M_bh)-5, np.log10(M_bh)+5 , gridsize) # in M_sun
    ...
    plt.text(0.5, 0.7, r'Prohibited', fontsize=20, ha='center', va='center', transform=plt.gca().transAxes)
    plt.text(0.5, 0.6, r'(Capture without collision is not possible)', fontsize=12, ha='center', va='center', transform=plt.gca().transAxes)

    cbar = plt.colorbar()
    cbar.set_label('Capture cross-section (AU$^2$)')
    plt.title(f'M_star = {"{:.2f}".format(M_star)}, M_bh = {"{:.2f}".format(M_bh)}')
