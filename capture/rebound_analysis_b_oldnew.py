...
folder = f'04.20/MbVinfA_Variation/rebound_sim_mp_d.py_1811' 
# Displays Time
...
OPmean = []
dframes = []
directoryp = "Plots/Capture/04.18/1508/McVinfImpB_Variation/"
...
for run, sim in matched_files:
    ...
    with fits.open(directoryf + folder + '/' + run, mode='update') as hdul:
        ...
        analysis_file = f"{directoryf}{str(s.strftime("%m.%d"))}/{script_name}_{current_time}/{mode}/rebound_{run}_analysis.fits"
        pr_hdu = fits.PrimaryHDU()
        pr_hdu.writeto(analysis_file, overwrite=True)
        hdul = fits.open(analysis_file, mode='update')
        particle_hdu = fits.BinTableHDU(Table(data=[A, B, C, T, E, SA], names=['xyzA', 'xyzB', 'xyzC', 'time', 'eC', 'aC']))
        particle_hdu.header['syst'] = syst
        particle_hdu.header['rClose'] = (r_close)
        particle_hdu.header['vInf'] = (v_inf)
        particle_hdu.header['v1primme'] = (v1prime)
        particle_hdu.header['bmin'] = (bmin)
        particle_hdu.header['bmax'] = (bmax)
        particle_hdu.header['initial_BC_distance'] = (initial_BC_distance)
        particle_hdu.header['mA'] = (mA)
        particle_hdu.header['mB'] = (mB)
        particle_hdu.header['mC'] = (mC)
        particle_hdu.header['rB'] = (rB)
        particle_hdu.header['rA'] = (rA)
        particle_hdu.header['aB'] = (aB)
        particle_hdu.header['opB'] = (opB)
        particle_hdu.header['totSimYr'] = (totYr)   
        particle_hdu.header['dt'] = (dt)
        particle_hdu.header['fSimYr'] = (fYr)
        particle_hdu.header['fBYr'] = (fBYr)
        particle_hdu.header['snpInt'] = (snpInt)
        particle_hdu.header['snpRt'] = (snpRt)
        
        hdul.append(particle_hdu)
        ...
        for statname, ll in zip(df_stats.columns[1:], l):
            ...
            l.append(len(status),find_first_event(status), find_last_event(status), find_stat_change(status)[-1], np.sum(status))
            
            try:
                print(f"longest status duration: {np.max(find_stat_change(status)[:][3])}")
                l.append(np.max(find_stat_change(status)[:][3]))
            except:
                l.append(0)

                     
            j += 1
        status_info_hdu = fits.BinTableHDU(Table(data=l, names=['Total Status Time', 'First Event', 'Last Event', 'Last Status Change', 'Total Status Time', 'Longest Status Duration']))   
...
fit_filename = f"{directoryf}{str(s.strftime("%m.%d"))}/{script_name}_{current_time}/{mode}/rebound_analysis_d_general_.fits"
hdu_pr = fits.PrimaryHDU()
hdu_pr.writeto(fit_filename, overwrite=True)
hdu = fits.open(fit_filename, mode='update')
hdu.append(fits.BinTableHDU(Table(data=[mPBH, vINF, ImpactB, Lifetime, SAmean, Emean], names=['mPBH', 'vINF', 'ImpactB', 'Lifetime', 'SAmean', 'Emean'])))
