...
run_dict = {run.split('_')[2].split('.')[0]: run for run in runs}  # Map run numbers to .fits files
sim_dict = {sim.split('_')[2].split('.')[0]: sim for sim in sims}  # Map sim numbers to .bin files
...
for run, sim in matched_files:
    ...
    SAmean.append(np.mean(SA))
    Emean.append(np.mean(E))
    OPmean = np.mean(opC)
    Beta.append(beta)
    ...
    ax[2].plot(T, opC, label='B', color='green', alpha=1)
    ax[2].set_ylabel('Orbital Period (yr)')
    ax[2].set_xlabel('Time (yr)')

    plt.suptitle(fr'$\beta$ = {beta}, $\lambda_1$= {lambda1}, b = {b}')
    ...
    j = 1
    df_stats = pd.DataFrame(S, columns=['Time', 'Free', 'Bcapture', 'Acapture', 'ABcapture'])
    for statname in df_stats.columns[1:]:
        ...
        S_info.append([df_stats.columns[j], find_first_event(status), find_last_event(status), find_stat_change(status)[-1], np.sum(status)])
        j += 1
    df_stats_info = pd.DataFrame(S_info, columns=['Status','First Event', 'Last Event', 'Last Status Change', 'Total Status Time'])

    dframes.append(df_stats_info)
    ...
    crosscond1 = np.logical_and(df_stats['Bcapture'] == 1, df_stats['Acapture'] == 1)
    crosscond2 = np.logical_and(df_stats['Bcapture'] == 1, df_stats['ABcapture'] == 1)
    crosscond3 = np.logical_and(df_stats['Bcapture'] == 1, df_stats['Free'] == 1)
    crosscond4 = np.logical_and(df_stats['Acapture'] == 1, df_stats['Free'] == 1)
    crosscond5 = np.logical_and(df_stats['ABcapture'] == 1, df_stats['Free'] == 1)
    crosscond6 = np.logical_and(df_stats['ABcapture'] == 1, df_stats['Acapture'] == 1)
