...
mode3 = 'MbVinfA_Variation'
mode = mode3
folder = '04.18/1508/' + mode
# Displays Time
...
os.makedirs(f"{directoryf}{script_name}_{current_time}/", exist_ok=True)
# # Configure logging
...
runs = [f for f in os.listdir("/data/a.saricaoglu/repo/COMPAS/Files/Capture/04.19/2047/MbVinfA_Variation") if ".fits" in f]
sims = [sim for sim in os.listdir("/data/a.saricaoglu/repo/COMPAS/Files/Capture/04.19/2047/MbVinfA_Variation") if ".bin" in sim]
...
for run, sim in matched_files:
    ...
    S = np.empty((0,5))
    S_info = []
    with fits.open('/data/a.saricaoglu/repo/COMPAS/Files/Capture/04.19/2047/MbVinfA_Variation/' + run, mode='update') as hdul:
        # hdul[1].data = Table(S)
        # hdul.flush()
        ...
        if b < bmin or b > bmax:
            ...
            continue
        archived_sim = rebound.Simulationarchive(f'/data/a.saricaoglu/repo/COMPAS/Files/Capture/04.19/2047/MbVinfA_Variation/{sim}')
        if fYr > 1e7:
...
fit_filename = f"{directoryf}{script_name}_{current_time}/rebound_analysis_d_general_.fits"
