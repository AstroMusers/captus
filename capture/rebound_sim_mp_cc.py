...
def run_simulation(pars):
    ...
    # Configure logging
    log_filename = f"{directoryf}{script_name}_{tr}_{i}_script.log"
    ...
    opDiff = opBc - opB
    print(f" Orbital period difference opBc - opB: {opBc}-{opB}={opDiff}")
    ...
    if bmin > bmax:
        print(f"bmin {bmin} > bmax {bmax}, skipping simulation")
        return "Skipping simulation due to bmin > bmax"
    ...
    snap_rate = 0.4  # snapshot every 10% of the time
    snapshot_interval = (sim.dt) # Number of steps to integrate
    ...
    sim.save_to_file(f"{directoryf}sim_{tr}_{i}_{mc}.bin",interval=snapshot_interval, delete_file=True)
    print(f"Simulation {i} {mc} saved to file: {directoryf}sim_{tr}_{i}_{mc}.bin")
    ...
    try:
        for k in range(0, int(t_end/step)):
            ...
            # if sim.t > t_max:
            #     raise MaxYearForBError(f"Simulation time exceeded maximum year limit for B years: {sim.t} > {t_max}")
            ...
            if yr == int(t_min/10):
                ...
                yr = 0
    except (EnergyError, EscapeError, CollisionError,MaxYearForBError) as e:
        print(f"Error during integration: {e}")
        print(f"Simulation failed for system {i} {mc}, at time {sim.t}")
        if sim.t < t_min:
            print(f"Time {sim.t} < t_min {t_min}")
            print(f"Checking if file exists: {directoryf}sim_{tr}_{i}_{mc}.bin")
            if not os.path.exists(f"{directoryf}sim_{tr}_{i}_{mc}.bin"):
                print(f"File not found: {directoryf}sim_{tr}_{i}_{mc}.bin")
            os.remove(f"{directoryf}sim_{tr}_{i}_{mc}.bin")
            print(f"File {directoryf}sim_{tr}_{i}_{mc}.bin has been deleted.")
            # destination_file = f"{directoryf_discarded}{script_name}_{tr}_{i}_script.log"
            ...
            print(f"Time {sim.t} > t_max {t_max}")
            return "Skipping to next system..."
        else:
            pass
    ...
    fit_filename = f"{directoryf}rebound_{tr}_{i}_{mc}.fits"
    ...
    # sa = rebound.Simulationarchive(f"{directoryf}sim_{tr}_{i}_{mc}.bin")
    # print(len(sa))
...
if __name__ == "__main__":
    ...
    j = 4
    num_cores = 62 # Get the number of available CPU cores
    bp = binary_picker.pick(j, 64.0) # Pick binaries with mass ratio 1.0
    target_ratio, mB, mA, rB, rA, aB, opBc = bp[0], bp[1], bp[2], bp[3], bp[4], bp[5], bp[6]
    with multiprocessing.Pool(processes=num_cores) as pool:
        # Distribute the simulations across the cores
        pars = []
        i = 0
        bmin = rB
        bmax = aB * (mB*0.1 / mA)**(1/3)
        for beta in np.linspace(-np.pi/2, np.pi/2, numSim):
            for lamda in np.linspace(0, 2*np.pi, numSim):
                for b in np.linspace(bmin, bmax, numSim): #bmin set to radius of B, B max set to r_close         
                    pars.append((j, i, beta, lamda, b))
                    i += 1
    ...
    # script_name = os.path.basename(__file__)
    # # Configure logging
    # log_filename = f"{directoryf}{script_name}_general_script.log"
    # os.makedirs(os.path.dirname(log_filename), exist_ok=True)
    # logging.basicConfig(filename=log_filename, level=logging.INFO, 
    #                     format='%(asctime)s - %(levelname)s - %(message)s')
    # # Print results
