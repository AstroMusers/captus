...
class MonteCarloSimulation:
    ...
    def _save_mc_results(self,mc_results, N):
        ...
        np.savez(f'{save_dir}/monte_carlo_results_v{results["v_inf"]/1e3:.1f}_s{seed}.npz',
                 ...
                 cap_anomalies=results['cap_anomalies'],
                 cap_inclinations=results['cap_inclinations'],
    ...
    def run_monte_carlo_simulation(self, v_inf):
        ...
        n_captured = 0
        out_lambda, out_beta, out_b, out_phi, out_anomalies, out_inclinations = [], [], [], [], [], []
        ...
        while sampled < N and quota_condition==False and (dt.datetime.now() - now).total_seconds() < self.max_execution_time:
            ...
            for lam, beta, phi in zip(lambda_samples, beta_samples, phi_samples):
                ...
                b_vec = calcs.b_vector(self.muB, v1primeMag, v1primeVec, phi, b)

                rABVec = calcs.r_AB_vec(self.aB, lam)
                r_B = rABVec + b_vec  # Vector from A to B
                # 2. Direction of v2 (normalized)
                v2_direction = v2Vec / v2Mag
                # 3. Position where PBH exits r_close sphere (traveling along v2 from B)
                exit_point = r_B + self.rClose * v2_direction
                ...
                if E2_val + (self.muB / self.rClose) < 0:
                    ...
                    if a_val > 0 and 0 <= e_val < (1 if self.e_lim is None else self.e_lim):
                        L_B = np.cross(exit_point, vBVec)
                        L_B_hat = L_B / np.linalg.norm(L_B)
                        L_vec = np.cross(exit_point, v2Vec)
                        ...
                        true_anomaly, eccentric_anomaly, mean_anomaly = calcs.anomalies(a_val, e_val, exit_point, v2Vec, self.muA)
                        out_a.append(a_val)
                        out_e.append(e_val)
                        ...
                        out_inclinations.append(rel_inclination_deg)
                        out_anomalies.append(true_anomaly)
        ...
        mc_results = {
            ...
            'cap_anomalies': np.array(out_anomalies),
            'cap_inclinations': np.array(out_inclinations),
