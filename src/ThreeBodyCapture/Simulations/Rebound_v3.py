...
class OrbitalSimulation:
    ...
    def _save_result(self, v, result):
        i = result[0]
        result_keys = ['i', 'v_inf', 'input_info', 'errors', 'start_distances', 'lifetime', 'integrations', 'final_energy_c', 'termination_flag', 'eccentricities', 'semi_major_axes', 'orbital_periods', 'times', 'collision_info', 'script_version']
    ...
    def run_orbital_integration(self, i, v_inf, lambda1, beta, phi, b, beta2, anomalies, a_c, e_c, check_exists=True):

        # if check_exists:
        #     if self._check_result_exists(v_inf / 1e3, i):
        #         print(f"Simulation {i} for v_inf {v_inf/1e3} km/s already exists. Skipping...")
        #         return
        input_info = {'i': i, 'v_inf': v_inf, 'lambda1': lambda1, 'beta': beta, 'phi': phi, 'b': b, 'beta2': beta2, 'anomalies': anomalies, 'a_c': a_c, 'e_c': e_c}
        ...
        sim.add(m=mC, a=aC, e=eC, r=rC, inc=iC, f=fC, primary=sim.particles[0])  # PBH
        ...
        result = [i, v_inf, input_info, error_list, start_separation, sim.t, j, E_c_cond, flag,
