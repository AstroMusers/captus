...
folder = f'05.01/1239/McVinfImpB_Variation/rebound_sim_mp_b_alt.py_1239' 
# Displays Time
...
for run, sim in matched_files:
    ...
    print(f'Processing run: {run}, sim: {sim} out of {len(matched_files)} starting at {datetime.datetime.now()}')
    A = []
    B = []
    C = []
    E = []
    ...
    with fits.open(directoryf + folder + '/' + run, mode='update') as hdul:
        ...
        if np.max(SA) > 120:
            print(f"System {syst} has a max semi-major axis greater than 120 AU, skipping...")
        ...
        # particle_hdu.header['inBCdist'] = (initial_BC_distance)
        ...
        # except:
        #     particle_hdu.header['totYr'] = (totYr)   
        #     particle_hdu.header['dt'] = (dt)
        #     particle_hdu.header['fYr'] = (fYr)
        #     particle_hdu.header['snpInt'] = (snpInt)
...
fit_filename = f"{directoryf}{str(s.strftime("%m.%d"))}/{script_name}_{current_time}/{mode}/rebound_analysis_b_general_.fits"
