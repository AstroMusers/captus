...
folder = f'04.30/1212/MbVinfA_Variation/rebound_sim_mp_d_alt.py_1212' 
# Displays Time
...
for run, sim in matched_files:
    ...
    print(f'Processing run: {run}, sim: {sim},  {i}/{len(matched_files)}')
    A = []
    B = []
    C = []
    E = []
...
fit_filename = f"{directoryf}{str(s.strftime("%m.%d"))}/{script_name}_{current_time}/{mode}/rebound_analysis_d_alt_general_.fits"
