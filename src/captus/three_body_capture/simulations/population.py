...
class PBHPopulation:
    ...
    def analyze_population(self, r=8, time_averages=False, load='All', update=False, use_cached_data=True, conservative_results=True, print_validation=False):
        """
        ...
        for p in list(self.population_dict.values()):
            ...
            analysis = anl.Analysis(name=configuration.name, configuration=configuration, load_rebound=True, load=load, conservative_results=conservative_results)
            ...
            if print_validation:
                print(f"Validation for configuration: {name}")
                analysis.print_system_matching_validation()
