...
for file in runs :
    ...
    if (np.sum(S['Acapture'])==0 and np.sum(S['ABcapture'])== 0):
        cap_check.append([f'system {syst} has no capture, skipping'])
        print(S['Acapture'])
        print(S['ABcapture'])
        continue
    ...
    if result[0] is not None:
        ...
        # --- Plotting
        # plt.figure(figsize=(7, 3.5))
        # plt.plot(T, E, label='Semi-major axis', color='coral')
        ...
        #                 color='none', alpha=1,edgecolor='rebeccapurple', label='Longest table region', hatch='xx')
        # plt.xlabel("Time")
        ...
        print("No stable region found.")
    fig, ax = plt.subplots(2,1,figsize=(3.5,7), sharex=True)
    ...
    plotInt = int((1/snpInt) * snpInt) * 100
    body_names = ["A", "B", "C"]
    marker = ["*", "v", "o"]
    sizes = [60, 20, 30]
    lwidths = [0.5, 0.5, 0.1]
    ...
    for body in [A, B, C]:
        ...
        #print(len(x))
        scplt = ax.scatter(x, y, c=T[::plotInt],cmap='plasma', marker=marker[n], s=sizes[n], edgecolors=edg_colors[n], linewidths=lwidths[n])  # Small dots for trails
        n += 1        
    ax.set_xlabel("X (AU)")
    ax.set_ylabel("Y (AU)")
    plt.colorbar(scplt, label='Time (yr)')
    ...
    # plt.rcParams.update({'font.size': 20})
    # ax.bar(time, S['Free'], label=f'Free ({S_info['F'][4]})', color='coral', alpha=0.5 )
...
# Beta = Beta.reshape([6, 5])
# Lambda = Lambda.reshape([6, 5])
# ImpactB = ImpactB.reshape([6, 5])
# SAmean = SAmean.reshape([6, 5])
# Emean = Emean.reshape([6, 5])
# plt.rcParams['lines.markersize'] = 50
# # Prepare the data array with Beta, Lambda, ImpactB, SAmean, and Emean
# data = np.vstack([Beta, Lambda, ImpactB, SAmean, Emean]).T  # Transpose to make each column a variable
...
#         r"$\bar{e}$",  # Emean
#     ],
#     range=[(-np.pi, np.pi), (0, 2 * np.pi), (0, 0.3), (0, 200), (0, 1)],
...
# data = np.vstack([Beta_aLimit, Lambda_aLimit, B_aLimit, SAmean_aLimit, Emean_aLimit]).T  # Transpose to make each column a variable
# figure = corner.corner(
#     data,
#     labels=[
#         r"$\beta$ (rad)",  # Beta
...
#         r"$\bar{e}$",  # Emean
#     ],
#     range=[(-np.pi, np.pi), (0, 2 * np.pi), (0, 0.3), (0, 120), (0, 1)],
#     quantiles=[0.16, 0.5, 0.84],  # Show 16th, 50th, and 84th percentiles
#     show_titles=True,  # Show titles with statistics
#     title_fmt=".2f",  # Format for the titles
#     title_kwargs={"fontsize": 12},  # Font size for titles
...
# data = np.vstack([Beta_stable_aLimit, Lambda_stable_aLimit, B_stable_aLimit, SAmean_stable_aLimit, Emean_stable_aLimit]).T  # Transpose to make each column a variable
# figure = corner.corner(
#     data,
#     labels=[
#         r"$\beta$ (rad)",  # Beta
...
#plt.rcParams.update({'font.size': 18})
ax = fig.add_subplot(111, projection='3d')
plot = ax.scatter((Beta_aLimit), Lambda_aLimit, B_aLimit, c=Lifetime_aLimit, marker='o', s=100)
plt.colorbar(plot, label='Lifetime (yr)', shrink=0.6, pad=0.2)
...
ax.set_zticks(np.linspace(np.min(B_aLimit), np.max(B_aLimit), 6), [f'{x:.3f}' for x in np.linspace(np.min(B_aLimit), np.max(B_aLimit), 6)])
ax.set_xlabel(r'$\beta$ (rad)', labelpad=20)
ax.set_ylabel(r'$\lambda$ (rad)',   labelpad=20)
ax.set_zlabel("b (AU)", labelpad=20)
...
bin_edges = 10
# counts, bin_edges = np.histogram(Beta, bins=8)
# bin_centers = (bin_edges[:-1] + bin_edges[1:]) / 2
# ax[0].hist(Beta, bins=bin_edges, color='coral', fill=True, label='All systems')
ax[0].hist(Beta_stable_aLimit, bins=bin_edges, facecolor='rebeccapurple', hatch='xx', edgecolor='rebeccapurple',  fill=False,label='Stable systems')
# ax[0].set_title(r"mPBH = " + f"{Betamean}")
ax[0].set_xlabel(r'$\beta$ (rad)')
# ax[0].set_ylabel('Number of systems')
#ax[0].set_xticks(bin_edges, labels=[f'{x:.2f}' for x in bin_edges])
# fig.text(0.85, 0.67, r"$\times 10^{-2}$",  rotation="horizontal")

 # Set x-ticks at bin centers
# ax[0].set_yscale('log')  # Set y-axis to logarithmic scale
# counts, bin_edges = np.histogram(Lambda, bins=8)
# bin_centers = (bin_edges[:-1] + bin_edges[1:]) / 2
# ax[1].hist(Lambda, bins=bin_edges, color='coral',  label='All systems')
ax[1].hist(Lambda_stable_aLimit, bins=bin_edges, facecolor='rebeccapurple', hatch='xx', edgecolor='rebeccapurple',  fill=False,label='Stable systems')
# ax[1].set_title(r"$v_\infty$ = " + f"{Lambdamean}")
ax[1].set_xlabel(r'$\lambda$ (rad)')
ax[1].set_ylabel('Number of systems')
#ax[1].set_xticks(bin_edges, labels=[f'{x:.2f}' for x in bin_edges])

# ax[1].set_yscale('log')  # Set y-axis to logarithmic scale

# counts, bin_edges = np.histogram(ImpactB, bins=8)
# bin_centers = (bin_edges[:-1] + bin_edges[1:]) / 2
# ax[2].hist(ImpactB, bins=bin_edges, color='coral',  label='All systems')
ax[2].hist(B_stable_aLimit, bins=bin_edges, facecolor='rebeccapurple', hatch='xx', edgecolor='rebeccapurple',  fill=False, label='Stable systems')
...
#ax[2].set_xticks(bin_edges, labels=[f'{x:.2f}' for x in bin_edges])
...
# formattery = LogFormatter(labelOnlyBase=True)  # Show all labels (not just 10^n)
formatterx = ScalarFormatter(useMathText=True)
formatterx.set_powerlimits((-2, 2))
# formatter.set_powerlimits((-2, 2)) 
for axx in ax:
    # axx.yaxis.set_major_formatter(formattery)
...
# counts, bin_edges = np.histogram(Beta_stable_aLimit, bins=20)
# bin_centers = (bin_edges[:-1] + bin_edges[1:]) / 2
...
# counts, bin_edges = np.histogram(Lambda_stable_aLimit, bins=20)
# bin_centers = (bin_edges[:-1] + bin_edges[1:]) / 2

# ax[1].hist(Lambda_stable_aLimit, bins=bin_edges, color='red')
# ax[1].set_title(r"$\lambda$ = " + f"{Lambdamean}")
# ax[1].set_xlabel(r'$\lambda$ (rad)')
# ax[1].set_ylabel('Number of systems')
# ax[1].set_xticks(bin_centers)  # Set x-ticks at bin centers
# counts, bin_edges = np.histogram(B_stable_aLimit, bins=20)
# bin_centers = (bin_edges[:-1] + bin_edges[1:]) / 2
# ax[2].hist(B_stable_aLimit, bins=bin_edges, color='green')
# ax[2].set_title(r"Impact Parameter = " + f"{bmean}")
# ax[2].set_xlabel(r"b (AU)")
# ax[2].set_ylabel('Number of systems')
...
# axins2 = inset_axes(ax[1], width=2, height=1)
...
# print(f'mean impact parameter: {np.mean(ImpactB)}')
# print(f"Mean lifetime: {np.mean(Lifetime)}")
# print (f'mean vINF: {np.mean(Lambda)}')
# print(f'mean mPBH: {np.mean(mC)}')
# print(f'median impact parameter: {np.median(ImpactB)}')
# print(f"Median lifetime: {np.median(Lifetime)}")
# print (f'median vINF: {np.median(Lambda)}')
# print(f'median mPBH: {np.median(mC)}')

print(f"Mean a limit  impact parameter: {np.mean(B_aLimit)}")
print(f"Mean a limit  lifetime: {np.mean(Lifetime_aLimit)}")
print(f"Mean a limit  vINF: {np.mean(Lambda_aLimit)}")
print(f"Mean a limit  mPBH: {np.mean(Beta_aLimit)}")
...
print(f"median a limit  impact parameter: {np.median(B_aLimit)}")
print(f"median a limit  lifetime: {np.median(Lifetime_aLimit)}")
print(f"median a limit  vINF: {np.median(Lambda_aLimit)}")
print(f"median a limit  mPBH: {np.median(Beta_aLimit)}")



# print(f"Mean a limit stable impact parameter: {np.mean(B_stable_aLimit)}")
# print(f"Mean a limit stable lifetime: {np.mean(Lifetime_stable_aLimit)}")
# print(f"Mean a limit stable vINF: {np.mean(Lambda_stable_aLimit)}")
# print(f"Mean a limit stable mPBH: {np.mean(Beta_stable_aLimit)}")
...
# print(f"median a limit stable impact parameter: {np.median(B_stable_aLimit)}")
# print(f"median a limit stable lifetime: {np.median(Lifetime_stable_aLimit)}")
# print(f"median a limit stable vINF: {np.median(Lambda_stable_aLimit)}")
# print(f"median a limit stable mPBH: {np.median(Beta_stable_aLimit)}")
# print(Beta_stable_aLimit)
# print(Lambda_stable_aLimit)
print(cap_check)
print(f"{len(Beta)} survived among {120**3} systems on the grid with stable systems under a limit: {len(Beta_stable_aLimit)} among all stable systems: {len(SAmean_stable)}")
