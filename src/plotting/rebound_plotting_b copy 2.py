...
for file in runs :
    ...
    # plt.title(fr'mPBH = {beta}, $v_\infty$= {lambda1}, b = {b}, $\tau$ = {fyr}')
...
# plt.savefig(f"{directoryp}mPBH_vInf_b_values.png", dpi=300)
# plt.close()

# fig = plt.figure(figsize=(7, 7))
# #plt.rcParams.update({'font.size': 18})
# ax = fig.add_subplot(111, projection='3d')
# plot = ax.scatter((mPBH_aLimit), vINF_aLimit, B_aLimit, c=Lifetime_aLimit, marker='o', s=100)
# plt.colorbar(plot, label='Lifetime (yr)', shrink=0.6, pad=0.2)
# ax.set_xticks(np.linspace(np.min(mPBH_aLimit), np.max(mPBH_aLimit), 6), [f'{x:.2f}' for x in np.linspace(np.min(mPBH_aLimit), np.max(mPBH_aLimit), 6)])
...
counts, bin_edges = np.histogram(mPBH, bins=8)
bin_centers = (bin_edges[:-1] + bin_edges[1:]) / 2
ax[0].hist(mPBH, bins=bin_edges, color='coral', fill=True, label='All systems')
ax[0].hist(mPBH_stable_aLimit, bins=bin_edges, facecolor='rebeccapurple', hatch='xx', edgecolor='rebeccapurple',  fill=False,label='Stable systems')
# ax[0].set_title(r"mPBH = " + f"{Betamean}")
ax[0].set_xlabel(r'M$_{PBH}$ (M$_\odot$)')
...
formattery = LogFormatter(labelOnlyBase=False)  # Show all labels (not just 10^n)
...
plt.tight_layout()
plt.savefig(f"{directoryp}histogram_mPBH_vInf_b.png", dpi=300, bbox_inches='tight')
plt.close()

counts, bin_edges = np.histogram(mPBH_stable_aLimit, bins=20)
bin_centers = (bin_edges[:-1] + bin_edges[1:]) / 2
...
print(f"Mean semimajor axis: {np.mean(SAmean)}")
print(f"Mean eccentricity: {np.mean(Emean)}")
print(f'median semimajor axis: {np.median(SAmean)}')
print(f'median eccentricity: {np.median(Emean)}')
...
print(f"Mean a limit  mPBH: {np.mean(mPBH_aLimit)}")
...
print(f"median a limit  mPBH: {np.median(mPBH_aLimit)}")
...
print(f"Mean a limit stable mPBH: {np.mean(mPBH_stable_aLimit)}")
...
print(f"median a limit stable mPBH: {np.median(mPBH_stable_aLimit)}")
# print(mPBH_stable_aLimit)
# print(vINF_stable_aLimit)
print(cap_check)
print(f"{len(mPBH)} survived among {120**3} systems on the grid with stable systems under a limit: {len(mPBH_stable_aLimit)} among all stable systems: {len(SAmean_stable)}")
