...
class OrbitalSimulation:
    ...
    def _save_result(self, v, result):
        i = result[0]
        result_keys = ['i', 'v_inf', 'start_info', 'errors', 'start_distances', 'lifetime', 'integrations','final_energy_c', 'termination_flag', 'eccentricities', 'semi_major_axes', 'orbital_periods', 'times']
    ...
    def run_orbital_integration(self, i, v_inf, lambda1, beta, phi, b, a_c, e_c):
        ...
        if result is None:
            result = [i, v_inf, start_info, error_list, start_separation, sim.t, j, E_c_cond, flag,
                      ...
                      np.array(orbital_periods, dtype=float), np.array(times, dtype=float)]
            self._save_result(v_inf_kms, result)
