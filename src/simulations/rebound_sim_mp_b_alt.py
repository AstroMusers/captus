# Sun-Jupiter- system, fixed lambda = 0 and beta = 0.013
# v_inf in interval 1 km/s to 100 km/s uniform random sampling pbh mass 10-13 Ms sun to 10^-4 msun
# Multiprocessing
import rebound
import numpy as np
...
if not os.path.exists(f'/data/a.saricaoglu/repo/COMPAS/Files/Capture/{str(s.strftime("%m.%d"))}/{current_time}/McVinfImpB_Variation/'): 
    os.makedirs(f'/data/a.saricaoglu/repo/COMPAS/Files/Capture/{str(s.strftime("%m.%d"))}/{current_time}/McVinfImpB_Variation/') 
directoryf = f'/data/a.saricaoglu/repo/COMPAS/Files/Capture/{str(s.strftime("%m.%d"))}/{current_time}/McVinfImpB_Variation/' 
...
def run_simulation(pars):
    ...
    e_b = 0.39785 # Eccentricity

    mA = 1.478 # mass of A in Msun
    mB = 0.592 # mass of B (jupiter) in Msun
    # mC = 1e-13 # mass of C in Msun
    rA = 2.043 * (const.R_sun)  # radius of A in AU
    rA = rA.to(u.au).value
    rB = 0.01234 * (const.R_sun)   # radius of B in AU
    rB = rB.to(u.au).value
    aB = 15   # semi-major axis of A-B in AU
    ...
    if (b < bmin) or (b > bmax) or (bmax < bmin):
        print(f"Impact parameter out of bounds: {b} not in [{bmin}, {bmax}]")            
    ...
    snap_rate = 3  # snapshot every 10% of the time
...
if __name__ == "__main__":
    ...
    num_cores = 55  # Get the number of available CPU cores
    with multiprocessing.Pool(processes=num_cores) as pool:
        # Distribute the simulations across the cores
        pars = []
        i = 0
        for mc in np.logspace(-6, 1, numSim):
            for vinf in np.linspace(0.2108, 21.08, numSim): # v_inf in au/yr
                for bb in np.linspace(0.0000574115, 5.132, numSim): #bmin set to radius of B, B max set to r_close         
                    pars.append((i, mc, vinf, bb))
                    i += 1
