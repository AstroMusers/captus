...
def run_simulation(pars):
    ...
    mB = 0.0009543 * mBj # mass of B (jupiter) in Msun
...
if __name__ == "__main__":
    ...
    with multiprocessing.Pool(processes=num_cores) as pool:
        ...
        pars = []
        i = 0
        for mb in np.logspace(-3, 3.02, numSim): #in jupiter mass, 0.0009543\cdot10^{3.02} = 0.99 m sun
            for vinf in np.linspace(0.2108, 21.08, numSim):
                for a in np.linspace(2*0.004652472, 100.0, numSim): #Rsun in au to 100 au        
