...
if not os.path.exists(f'/data/a.saricaoglu/repo/COMPAS/Files/Capture/{str(s.strftime("%m.%d"))}/{current_time}/MbVinfA_Variation/'): 
    os.makedirs(f'/data/a.saricaoglu/repo/COMPAS/Files/Capture/{str(s.strftime("%m.%d"))}/{current_time}/MbVinfA_Variation/') 
directoryf = f'/data/a.saricaoglu/repo/COMPAS/Files/Capture/{str(s.strftime("%m.%d"))}/{current_time}/MbVinfA_Variation/' 
...
def run_simulation(pars):
    ...
    mA = 196 # mass of A in Msun R136a1
    mB =  mBj # mass of B in Msun
    mC = 1 # mass of C in Msun
    rA = 42.7 * (const.R_sun)  # radius of A in AU
    rA = rA.to(u.au).value
    rB = mB**0.8  * (const.R_sun)   # radius of B in AU
    rB = rB.to(u.au).value
    aB = aB + rB + rA   # semi-major axis of A-B in AU
    ...
    try:
        ...
        if (b < bmin) or (b > bmax) or (bmax < bmin):
            raise RangeError(f"Impact parameter out of bounds: {b} not in [{bmin}, {bmax}]")
    except RangeError as e:
        print(f"Error in impact parameter: {e}")
...
if __name__ == "__main__":
    ...
    with multiprocessing.Pool(processes=num_cores) as pool:
        ...
        pars = []
        i = 0
        for mb in np.logspace(0, 2.29, numSim): #in solar masses
            for vinf in np.linspace(0.2108, 21.08, numSim):
                for a in np.linspace(0.1, 100.0, numSim): #Rsun in au to 100 au        
                    pars.append((i, mb, vinf, a))
                    i += 1
